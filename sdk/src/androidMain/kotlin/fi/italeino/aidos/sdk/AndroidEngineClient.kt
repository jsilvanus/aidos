 ...
 * deep-link intent Engine returns while the caller is `PENDING_APPROVAL` (RFC-0103, "Trust
 * model"). Kept off the shared interface because [PendingIntent] doesn't exist on the jvm()
 * target sdk/ also builds for.
 */
class AndroidEngineClient internal constructor(
    ...
    /**
     * Deep-link intent to Engine's ConnectedAppsScreen from the most recent handshake, if that
     * handshake came back `PENDING_APPROVAL`. Present the caller with this (RFC-0103's
     * "Degradation" section) instead of re-polling `initialize()`.
     */
 ...
 * (RFC-0103, Phase C.3). This is the entry point Android apps use; [AidosEngineClientFactory] in
 * the shared source set stays Binder-free for the jvm() target.
 */
object AndroidAidosEngineClientFactory {
     ...
     */
    fun createClient(context: Context): AndroidEngineClient {
        val handshake = EngineBinderHandshake(context.applicationContext)
        return AndroidEngineClient(EngineClientImpl(handshake), handshake)
    }
}
