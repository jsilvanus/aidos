...
| **M25** | Retention and compaction | 0056, 0045 | Storage per active project stays under 512MB after 90 simulated days of use with default retention. Compaction is idempotent and runs in bounded batches with cancellation checks (RFC-0056) — an interruption redoes at most one in-flight batch on the next pass, not the whole backlog, and never corrupts state |
