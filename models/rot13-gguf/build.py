#!/usr/bin/env python3
"""Build `rot13.gguf` — a deterministic byte-level ROT13 model in GGUF.
...
exactly ROT13 of the last token for every one of the 256 possible inputs — which
is what makes it useful as a smoke-test fixture: any wrong byte is a runtime bug.
...
logits are 16 at row rot13(t) and 0 everywhere else — a decisive argmax with a
gap far wider than any quantization or fp16 error.
...
any bytes and it answers with ROT13 of the last one, then of that answer, and so
on -- and since ROT13 is an involution the stream alternates, which is an easy
property to assert on.
...
def build(path: pathlib.Path) -> None:
    ...
    writer.add_name("rot13")
    writer.add_description(
        "Hand-built byte-level ROT13 transformer; smoke-test fixture, not trained."
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "--output",
        type=pathlib.Path,
        default=pathlib.Path(__file__).with_name("rot13.gguf"),
    )
