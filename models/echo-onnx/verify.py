#!/usr/bin/env python3
"""Verify `echo.onnx` against a reference identity implementation.
...
def main() -> int:
    path = pathlib.Path(sys.argv[1]) if len(sys.argv) > 1 else pathlib.Path(__file__).with_name("echo.onnx")
    ...
    # 3. The identity function is trivially its own inverse.
