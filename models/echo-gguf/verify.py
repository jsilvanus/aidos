#!/usr/bin/env python3
"""Verify `echo.gguf` — structurally, then numerically, then (if available) for real.
...
Three layers, each independent of the one below it:
  ...
  2. Run a reference llama forward pass in numpy over all 256 tokens and assert
     the argmax is the identity. This is the layer that proves the *weights*
     are right, and it needs nothing but numpy.
...
def reference(data: bytes) -> bytes:
    """Identity: the model must echo its input back unchanged."""
    return data


def main() -> int:
    path = pathlib.Path(sys.argv[1]) if len(sys.argv) > 1 else pathlib.Path(__file__).with_name("echo.gguf")
    failures: list[str] = []
    ...
    # --- Layer 2: the weights actually compute the identity -----------------
    ...
    # position should still predict the identity of its own token.
    ...
    try:
        ...
        # Greedy decode: the identity function is a fixed point, so free-running
        # generation just repeats the last prompt byte forever.
        llm.reset()
        produced = bytearray()
        for token in llm.generate(tokens, temp=0.0):
            ...
            if len(produced) == 4:
                break
        check("llama.cpp decode", bytes(produced), bytes([text[-1]] * 4))
